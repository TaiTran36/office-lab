...
function wpbeginner_numeric_shop($max_num_pages, $paged, $url, $search)
{
    ...
    if (!in_array(1, $links)) {
        $class = 1 == $paged ? ' class="active"' : '';
        printf('<li%s><a href="%s" ' . ($paged == 1 ? $colorPick : $color) . '>%s</a></li>' . "\n", $class, esc_url($url) . (empty($search) ? '' : $search), 1);
    ...
    foreach ((array)$links as $link) {
        $class = $paged == $link ? ' class="active"' : '';
        printf('<li%s><a href="%s" ' . ($paged == $link ? $colorPick : $color) . '>%s</a></li>' . "\n", $class, esc_url($url . '/page/' . $link . '/') . (empty($search) ? '' : $search), $link);
    }
    ...
    if (!in_array($max, $links)) {
        ...
        $class = $paged == $max ? ' class="active"' : '';
        printf('<li%s><a href="%s" ' . ($paged == $max ? $colorPick : $color) . '>%s</a></li>' . "\n", $class, esc_url($url . '/page/' . $max . '/') . (empty($search) ? '' : $search), $max);
    }
...
function cw_change_product_price_display( $price ) {
    ...
    if (count($price_array) == 1) {
        ...
        foreach ($price_array as $key => $amount) {
            ...
            $price_original[$key] 	= get_woocommerce_currency_symbol( $args['currency']) . number_format_i18n($original_price, 0);
            $price_discounted[$key] = get_woocommerce_currency_symbol( $args['currency']) . number_format_i18n($discounted_price, 0);
